...
void root(const uchar4 *in, uchar4 *out, uint32_t x, uint32_t y) {
    ...
    if(x==0 || x==239 || y==0 || y==269){
        ...
        //upper middle pixel
        float3 pixelUM = convert_float4(rsGetElementAt_uchar4(inPixels, x, y-1)).rgb;
        threeBythree[1] = (pixelUM.r + pixelUM.g + pixelUM.b)/3;

        //top right pixel
        float3 pixelTR = convert_float4(rsGetElementAt_uchar4(inPixels,  x+1, y+1)).rgb;
        threeBythree[2] = (pixelTR.r + pixelTR.g + pixelTR.b)/3;

        // middle left pixel
        float3 pixelML = convert_float4(rsGetElementAt_uchar4(inPixels,  x-1, y)).rgb;
        threeBythree[3] = (pixelML.r + pixelML.g + pixelML.b)/3;

        //middle right pixel
        float3 pixelMR = convert_float4(rsGetElementAt_uchar4(inPixels, x+1, y)).rgb;
        threeBythree[4] = (pixelMR.r + pixelMR.g + pixelMR.b)/3;

        //middle center pixel (the same as pixel)
        float3 pixelMC = convert_float4(rsGetElementAt_uchar4(inPixels, x, y)).rgb;
        threeBythree[5] = (pixelMC.r + pixelMC.g + pixelMC.b)/3;

        //bottom left pixel
        float3 pixelBL = convert_float4(rsGetElementAt_uchar4(inPixels, x-1, y+1)).rgb;
        threeBythree[6] = (pixelBL.r + pixelBL.g + pixelBL.b)/3;

        //bottom middle left pixel
        float3 pixelBML = convert_float4(rsGetElementAt_uchar4(inPixels, x, y+1)).rgb;
        threeBythree[7] = (pixelBML.r + pixelBML.g + pixelBML.b)/3;

        //bottom right pixel
        float3 pixelBR = convert_float4(rsGetElementAt_uchar4(inPixels, x+1, y+1)).rgb;
